...
//  BaseTrunk
//
//  Created by liuxianglong on 15/8/8.
//  Copyright (c) 2015年 wang yong. All rights reserved.
//


#import "QJCrashFoundation.h"
#import "FMLogger.h"

@implementation  QJCrashFoundation
...
- (id)_objectAtIndex:(NSUInteger)index
{
    if (index >= [self count])
    {
        FMLoggerError(@"%@",[NSString stringWithFormat:@"index[%ld] >= count[%ld]", index, [self count]]);
        return nil;
    }
...
- (void)_insertString:(NSString *)sString atIndex:(NSUInteger)index
{
    ...
    {
        FMLoggerError(@"%@",[NSString stringWithFormat:@"index[%ld] > length[%ld]", index, [self length]]);
        return;
    }
